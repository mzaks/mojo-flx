...
struct FlxBuffer[dedup_string: Bool = True, dedup_key: Bool = True, dedup_keys_vec: Bool = True](Copyable, Movable):
    ...
    var _key_cache: _CacheStringOrKey[False]
    ...
    fn __init__(inout self, size: UInt64 = 1 << 11):
        ...
        self._key_cache = _CacheStringOrKey[is_string=False]()
    ...
    fn _add_string[as_key: Bool](inout self, value: String):
        ...
        let bit_width = ValueBitWidth.of(byte_length << 2)
        ...
        if dedup_string and not as_key:
            ...
            if cached_offset != -1:
                self._stack.push_back(StackValue(bitcast[DType.uint8, 8](Int64(cached_offset)), ValueBitWidth.width8, ValueType.String))
                return
    ...
    fn blob(inout self, value: DTypePointer[DType.uint8], length: Int):
        let offset = self._offset
        let bit_width = ValueBitWidth.of(length << 2)
        let byte_width = self._align(bit_width)
    ...
    fn add[D: DType](inout self, value: DTypePointer[D], length: Int):
        let len_bit_width = ValueBitWidth.of(length << 2)
        let elem_bit_width = ValueBitWidth.of(SIMD[D, 1](0))
        let offset = self._offset
        let len_byte_width = self._align(len_bit_width)    
        ...
        self._offset = new_offest
        self._stack.push_back(StackValue(bitcast[DType.uint8, 8](offset), elem_bit_width, ValueType.of[D]() + ValueType.Vector))
    ...
    fn _create_vector(inout self, start: Int, length: Int, step: Int, keys: StackValue = StackValue.Null) raises -> StackValue:
        ...
        let offset = self._offset
        let length_bit_width = ValueBitWidth.of(UInt64(length << 2))
        let length_byte_width = self._align(length_bit_width)
